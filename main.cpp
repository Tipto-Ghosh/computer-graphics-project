// ═══════════════════════════════════════════════════════════════════════════════
//   Village Scenery  –  Enhanced Edition v2
...
// ══════════════════════════════════════════════════════════════════════════════
//  SCENE SECTION FUNCTIONS  (called in back-to-front order by DrawAllComponents)
// ══════════════════════════════════════════════════════════════════════════════
...
// 2. Sun – three glow rings + core + rotating rays
void DrawSun()
{
...
// 3. Three drifting cloud formations
void DrawClouds()
{
...
// 4. Far mountains with snow caps + peak highlights
void DrawMountains()
{
    // Mountain body
...
// 5. Near hills – three layered polygons with grass texture patches on crests
void DrawHills()
{
...
// 6. River water body + animated wave ripples + bright highlight streaks
void DrawRiver()
{
    ...
    // Bright highlight ripple streaks
...
// 7. Ground + river-land bank differentiator (wet mud → sandy bank → grass
//    transition with reed row) + dirt path
void DrawGround()
{
    ...
    // Reed/cattail row along the bank
    ...
    for (int i = 0; i < reedN; i++) {
        ...
        glEnd();
        // Cattail seed-head (dark brown, scaled ellipse via sphere+scale)
        glColor3ub(88, 52, 16);
        glPushMatrix();
    ...
    // ── Dirt path from fence gate down to the river bank ───────────────────────
...
// 9. Wildflower patches (stem + sphere bloom)
void DrawFlowers()
{
...
// 10. Both village houses + chimney smoke
void DrawAllHouses()
{
    DrawHouse(-2.0f, -7.2f);             // House 1 (left)
    DrawHouse(11.0f, -9.2f);             // House 2 (right)
    DrawSmoke(-2.0f + 1.75f, -7.2f + 7.75f);   // House 1 chimney top
    DrawSmoke(11.0f + 1.75f, -9.2f + 7.75f);   // House 2 chimney top
}

// 11. Four pine trees along the right bank
void DrawAllTrees()
{
...
// 12. Fence posts + horizontal rails (gate gap at dirt path)
void DrawFence()
{
    for (float fx = -11.5f; fx <= 36.5f; fx += 2.2f) {
        if (fx > -2.8f && fx < 0.8f) continue;   // gate gap
...
// 13. Railway tracks + sleepers (perspective-foreshortened)
void DrawRailway()
{
    // Upper rail
...
// 15. Boat 1 (L→R sailboat) and Boat 2 (R→L cabin cruiser)
void DrawBoats()
{
    // ── Boat 1 ────────────────────────────────────────────────────────────────
    glPushMatrix();
...
void DrawAllComponents()
{
    ...
    DrawSun();        //  2. Sun + rotating rays
    DrawClouds();     //  3. Drifting clouds
    DrawMountains();  //  4. Far mountains + snow caps
    DrawHills();      //  5. Layered near hills with crest detail
    DrawRiver();      //  6. River water + animated waves
    DrawGround();     //  7. Ground + river-bank differentiator + reeds + path
    DrawBushes();     //  8. Bush clusters
    DrawFlowers();    //  9. Wildflowers
    DrawAllHouses();  // 10. Village houses + chimney smoke
    DrawAllTrees();   // 11. Professional pine trees
    DrawFence();      // 12. Fence + rails
    DrawRailway();    // 13. Railway tracks + sleepers
    DrawBirds();      // 14. Flying bird flock
    DrawBoats();      // 15. Animated boats on river
